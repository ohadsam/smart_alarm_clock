...
| 143 | כפתור רענון (⟳) בכל ארבעת הגדלים; עובד גם על ווידג'ט תקוע כי מטפלי ההקשה שרדו ברינדור האחרון | `RefreshWidgetAction` (v1.12.6) |
| 144 | רצועת ה-2x2 ב-22dp (לא 26dp) — שלושה אייקונים ב-94dp שימושיים, מול ארבע שורות תוכן | `SmallBody` (v1.12.6) |
| 145 | **`provideGlance` רץ פעם אחת ל-session, לא לכל עדכון.** מצב שנקרא לפני `provideContent` קפוא לנצח | `SmartRingBaseWidget` (v1.12.7) |
| 146 | הנתונים נקראים בתוך התצוגה מזרם חי; שינוי ב-Room או ב-DataStore מצייר מחדש לבד | `widgetStateFlow` (v1.12.7) |
| 147 | `provideGlance` הוא `final`; גודל מצהיר רק על `Body` — כדי שגודל אחד לא יחזיר את הבאג | `SmartRingBaseWidget` (v1.12.7) |
| 148 | `update()` שהסתיים **אינו** אומר שרונדר; כש-session חי הוא רק מצייר מחדש את התצוגה הקיימת | `refreshAllWidgets` (v1.12.7) |
