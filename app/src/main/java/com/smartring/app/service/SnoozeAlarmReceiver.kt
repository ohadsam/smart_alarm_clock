...
class SnoozeAlarmReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(ctx: Context, intent: Intent) {
        ...
        goAsync().also { p -> CoroutineScope(Dispatchers.IO).launch {
            ...
            if (alarm == null) { ctx.stopService(Intent(ctx, AlarmFiringService::class.java)); p.finish(); return@launch }
            ...
            if (!alarm.acceptsInteraction) { p.finish(); return@launch }
