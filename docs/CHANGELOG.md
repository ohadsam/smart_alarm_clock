...
### Room's exported schemas

`app/schemas/` is generated by KSP and had never been committed, so a schema change was
invisible in a code review. Being worked on — the JSON can only be produced by a real
Android build, which this environment cannot run, so it is dumped through the CI log and
